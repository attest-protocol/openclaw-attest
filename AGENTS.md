...
- **Parameters are never stored plaintext by default** — only SHA-256 hashes in receipts. When `parameterDisclosure` is explicitly enabled, named fields are stored verbatim alongside the hash — this is an opt-in disclosure, not a default.
